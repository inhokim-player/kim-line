...
export default function Dashboard({ onExitToLanding, initialResult }) {
  ...
  function handleQuizComplete({ metrics }) {
    setValues(metrics)
    persistLog(metrics)
    setRevealData({ metrics })
    setMode('reveal')
  }
    ...
    <div className="min-h-screen bg-canvas pb-16">
      <header className="px-6 pt-8 pb-2 flex items-center justify-between">
        <div>
          ...
          {streak > 1 && (
            <p className="text-xs text-pulse-600 mt-1 font-medium">🔥 연속 {streak}일째 기록 중</p>
          )}
        </div>
      ...
      <main className="px-6 mt-4 space-y-4 max-w-md mx-auto">
        <div className="rounded-card bg-surface p-6 flex flex-col items-center">
        ...
        <WeakPointsCard values={values} />

        <button
          onClick={() => setMode('stats')}
          className="w-full rounded-card bg-surface p-4 flex items-center justify-between hover:bg-canvas transition"
        >
